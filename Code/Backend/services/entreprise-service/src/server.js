...
async function startServer() {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log("Connexion PostgreSQL OK (entreprise-service).");
      lastError = null;
      break;
    } catch (err) {
      lastError = err;
  ...
  app.listen(PORT, () => {
    console.log(`Entreprise Service écoute sur le port ${PORT}`);
  });
}
...
if (require.main === module) {
  startServer().catch((err) => {
    console.error("Démarrage impossible (entreprise-service):", err);
    process.exit(1);
  });
}
